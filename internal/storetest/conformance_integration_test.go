...
func TestStore_Postgres_Conformance(t *testing.T) {
	...
	newStore := func(t *testing.T) store.Store {
		...
		st, err := sqlstore.OpenPostgres(pg.DSN, tokenKey, sqlstore.WithTablePrefix(prefix))
		if err != nil {
	...
	runConformanceSuite(t, newStore)
}

func TestStore_MySQL_Conformance(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()

	my, err := containers.StartMySQL(ctx)
	if err != nil {
		t.Fatalf("StartMySQL: %v", err)
	}
	defer func() { _ = my.Terminate(context.Background()) }()
	...
	newStore := func(t *testing.T) store.Store {
		t.Helper()
		prefix := fmt.Sprintf("t%d", time.Now().UnixNano())
		st, err := sqlstore.OpenMySQL(my.DSN, tokenKey, sqlstore.WithTablePrefix(prefix))
		if err != nil {
...
func TestStore_Mongo_Conformance(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()

	mg, err := containers.StartMongoReplicaSet(ctx)
	if err != nil {
		t.Fatalf("StartMongoReplicaSet: %v", err)
	}
	defer func() { _ = mg.Terminate(context.Background()) }()
	...
	newStore := func(t *testing.T) store.Store {
		t.Helper()
		prefix := fmt.Sprintf("t%d", time.Now().UnixNano())
		st, err := mongostore.Open(ctx, mongostore.OpenConfig{
			URI:      mg.URI,
			Database: mg.Database,
			TokenKey: tokenKey,
			Options:  []mongostore.Option{mongostore.WithCollectionPrefix(prefix)},
		})
		if err != nil {
...
func runConformanceSuite(t *testing.T, newStore func(t *testing.T) store.Store) {
	t.Helper()

	t.Run("MerchantLifecycle", func(t *testing.T) {
		st := newStore(t)
	...
	t.Run("MerchantWalletImmutable", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
	...
	t.Run("CreateInvoiceIdempotent", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
		...
		req2 := req
		req2.AmountZat = 11
		if _, _, err := st.CreateInvoice(ctx, req2); err != store.ErrConflict {
			t.Fatalf("expected ErrConflict for different amount, got %v", err)
		}
	})

	t.Run("CreateInvoiceAddressUniquePerWallet", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
		...
		if _, _, err := st.CreateInvoice(ctx, req1); err != nil {
			t.Fatalf("CreateInvoice1: %v", err)
		}

		req2 := req1
		req2.ExternalOrderID = "order-2"
		if _, _, err := st.CreateInvoice(ctx, req2); err != store.ErrConflict {
			t.Fatalf("expected ErrConflict for duplicate wallet address, got %v", err)
		}
	})

	t.Run("APIKeyLifecycle", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
	...
	t.Run("InvoiceToken", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
		...
		got, ok, err := st.GetInvoiceToken(ctx, inv.InvoiceID)
		if err != nil {
	...
	t.Run("ListInvoicesFilterAndCursor", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
		...
		i1, _, err := st.CreateInvoice(ctx, store.InvoiceCreate{
			MerchantID:       m.MerchantID,
			ExternalOrderID:  "order-1",
			WalletID:         "w1",
			AddressIndex:     0,
		...
		i2, _, err := st.CreateInvoice(ctx, store.InvoiceCreate{
			MerchantID:       m.MerchantID,
		...
		page2, cur2, err := st.ListInvoices(ctx, store.InvoiceFilter{MerchantID: m.MerchantID, AfterID: cur1, Limit: 10})
		if err != nil {
		...
		byExt, _, err := st.ListInvoices(ctx, store.InvoiceFilter{MerchantID: m.MerchantID, ExternalOrderID: "order-1", AfterID: 0, Limit: 10})
		if err != nil {
	...
	t.Run("ListDepositsFilterAndCursor", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
		...
		page1, cur1, err := st.ListDeposits(ctx, store.DepositFilter{MerchantID: m.MerchantID, AfterID: 0, Limit: 1})
		if err != nil {
		...
		page2, _, err := st.ListDeposits(ctx, store.DepositFilter{MerchantID: m.MerchantID, AfterID: cur1, Limit: 10})
		if err != nil {
		...
		onlyInv, _, err := st.ListDeposits(ctx, store.DepositFilter{MerchantID: m.MerchantID, InvoiceID: inv.InvoiceID, AfterID: 0, Limit: 10})
		if err != nil {
	...
	t.Run("RefundsAndOutbox", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
		...
		r1, err := st.CreateRefund(ctx, store.RefundCreate{
			MerchantID: m.MerchantID,
		...
		r2, err := st.CreateRefund(ctx, store.RefundCreate{
			MerchantID: m.MerchantID,
			InvoiceID:  inv.InvoiceID,
			ToAddress:  "j1dest2",
			AmountZat:  2,
		...
		refunds, _, err := st.ListRefunds(ctx, store.RefundFilter{MerchantID: m.MerchantID, AfterID: 0, Limit: 10})
		if err != nil {
		...
		sentOnly, _, err := st.ListRefunds(ctx, store.RefundFilter{MerchantID: m.MerchantID, Status: domain.RefundSent, AfterID: 0, Limit: 10})
		if err != nil {
	...
	t.Run("ReviewCasesUnknownAddressDeposit", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
	...
	t.Run("ReviewCasesFromInvoicePolicies", func(t *testing.T) {
		st := newStore(t)
		ctx := context.Background()
		...
		invPartial, _, err := st.CreateInvoice(ctx, store.InvoiceCreate{
			MerchantID:       m.MerchantID,
		...
		invOverpaid, _, err := st.CreateInvoice(ctx, store.InvoiceCreate{
			MerchantID:       m.MerchantID,
		...
		invLate, _, err := st.CreateInvoice(ctx, store.InvoiceCreate{
			MerchantID:       m.MerchantID,
